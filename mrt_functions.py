...
def returnMayaVersion():
    ...
    """
    # return int(eval(mel.eval('$temp = $gMayaVersionYear')))    # Alternative
...
def createSkeletonFromModule(moduleAttrsDict, characterName):
    ...
    if moduleAttrsDict.get('mirror_module_Namespace') != None and moduleAttrsDict['creation_plane'][0] == '-':
        ...
        rootJointName = 'MRT_character'+characterName+'__'+moduleAttrsDict['userSpecName']+'_root_node_transform'
        ...
        r_joints = cmds.mirrorJoint(origRootJointName, mirrorBehavior=rotationFunc['behaviour'], mirrorXY=mirrorAxis['XY'], mirrorXZ=mirrorAxis['XZ'], mirrorYZ=mirrorAxis['YZ'])
...
def createProxyForSkeletonFromModule(characterJointSet, moduleAttrsDict, characterName):
        ...
        for (i, joint) in zip(range(moduleAttrsDict['num_nodes']), jointSet):
            if index == 0:
                ...
                if cmds.objExists(moduleAttrsDict['module_Namespace']+':root_node_transform_proxy_bone_geo_preTransform'):
                    ...
                    cmds.parentConstraint(joint, bone_proxy, maintainOffset=True, name=joint+'_root_node_transform_proxy_bone_geo_preTransform_parentConstraint')
                    ...
                    cmds.scaleConstraint(joint, bone_proxy, maintainOffset=True, skip=moduleAttrsDict['node_axes'][0].lower(), name=joint+'_root_node_transform_proxy_bone_geo_preTransform_scaleConstraint')
                    cmds.parent(bone_proxy, proxyGrp)

            elif index == moduleAttrsDict['num_nodes']-1:
                if cmds.objExists(moduleAttrsDict['module_Namespace']+':end_node_transform_proxy_elbow_geo_preTransform'):
                    elbow_proxy = cmds.duplicate(moduleAttrsDict['module_Namespace']+':end_node_transform_proxy_elbow_geo_preTransform', returnRootsOnly=True, renameChildren=True, name='MRT_character'+characterName+'__'+moduleAttrsDict['userSpecName']+'_end_node_transform_proxy_elbow_geo_preTransform')[0]
                    cmds.makeIdentity(elbow_proxy, scale=True, apply=True)
                    cmds.parentConstraint(joint, elbow_proxy, maintainOffset=True, name=joint+'_end_node_transform_proxy_elbow_geo_preTransform_parentConstraint')
                    cmds.scaleConstraint(joint, elbow_proxy, maintainOffset=True, name=joint+'_end_node_transform_proxy_elbow_geo_preTransform_scaleConstraint')
                    cmds.parent(elbow_proxy, proxyGrp)
            else:
                if cmds.objExists(moduleAttrsDict['module_Namespace']+':node_%s_transform_proxy_elbow_geo_preTransform'%(index)):
                    elbow_proxy = cmds.duplicate(moduleAttrsDict['module_Namespace']+':node_%s_transform_proxy_elbow_geo_preTransform'%(index), returnRootsOnly=True, renameChildren=True, name='MRT_character'+characterName+'__'+moduleAttrsDict['userSpecName']+'_node_%s_transform_proxy_elbow_geo_preTransform'%(index))[0]
                    cmds.makeIdentity(elbow_proxy, scale=True, apply=True)
                    cmds.parentConstraint(joint, elbow_proxy, maintainOffset=True, name=joint+'_node_%s_transform_proxy_elbow_geo_preTransform_parentConstraint'%(index))
                    cmds.scaleConstraint(joint, elbow_proxy, maintainOffset=True, name=joint+'_node_%s_transform_proxy_elbow_geo_preTransform_scaleConstraint'%(index))
                    cmds.parent(elbow_proxy, proxyGrp)

                if cmds.objExists(moduleAttrsDict['module_Namespace']+':node_%s_transform_proxy_bone_geo_preTransform'%(index)):
                    bone_proxy = cmds.duplicate(moduleAttrsDict['module_Namespace']+':node_%s_transform_proxy_bone_geo_preTransform'%(index), returnRootsOnly=True, renameChildren=True, name='MRT_character'+characterName+'__'+moduleAttrsDict['userSpecName']+'_node_%s_transform_proxy_bone_geo_preTransform'%(index))[0]
                    cmds.makeIdentity(bone_proxy, scale=True, apply=True)
                    cmds.parentConstraint(joint, bone_proxy, maintainOffset=True, name=joint+'_node_%s_transform_proxy_bone_geo_preTransform_parentConstraint'%(index))
                    distanceBetweenNode = cmds.createNode('distanceBetween', name=bone_proxy+'_scaleLengthDistance', skipSelect=True)
                    distanceBetween_1_vecPro = cmds.createNode('vectorProduct', name=bone_proxy+'_scaleLengthVector_1_point', skipSelect=True)
                    distanceBetween_2_vecPro = cmds.createNode('vectorProduct', name=bone_proxy+'_scaleLengthVector_2_point', skipSelect=True)
                    distanceScaleDivide = cmds.createNode('multiplyDivide', name=bone_proxy+'_distanceScaleDivide', skipSelect=True)
                    ...
                    cmds.setAttr(distanceScaleDivide+'.input2X', defDistance)
                    cmds.connectAttr(distanceScaleDivide+'.outputX', bone_proxy+'.scale'+moduleAttrsDict['node_axes'][0].upper())
                    cmds.scaleConstraint(joint, bone_proxy, maintainOffset=True, skip=moduleAttrsDict['node_axes'][0].lower(), name=joint+'_node_%s_transform_proxy_bone_geo_preTransform_scaleConstraint'%(index))
                    cmds.parent(bone_proxy, proxyGrp)
            index += 1
    ...
    if allChildren:
        for node in allChildren:
            if cmds.objectType(node, isAType='constraint'):
                ...
                if not connections:
                    cmds.delete(node)
                continue
            nodeName = node.rpartition('|')
            if nodeName[2][:3] != 'MRT':
                cmds.rename(node, 'MRT_character'+characterName+'__'+moduleAttrsDict['userSpecName']+'_'+nodeName[2])
        allproxyTransforms = cmds.listRelatives(proxyGrp, allDescendents=True, type='transform')
        proxy_geo_transforms = [item for item in allproxyTransforms if str(item).rpartition('geo')[2] == '']
        ...
        for transform in proxy_geo_transforms:
            cmds.polyColorPerVertex(transform+'.vtx[*]', alpha=1.0, rgb=[0.663, 0.561, 0.319], notUndoable=True, colorDisplayOption=True)
            cmds.editDisplayLayerMembers(layerName, transform)
        return proxyGrp, distanceScaleInitMultNodes
    else:
        cmds.delete(proxyGrp)
        return None

def createFKlayerDriverOnJointHierarchy(characterJointSet, jointLayerName, characterName, asControl=False, layerVisibility='On', transFilter=False, controlLayerColour=None, connectLayer=True):
    """
    ...
    if connectLayer:
        for (joint, layerJoint) in zip(sorted(characterJointSet), sorted(layerJointSet)):
            parentConstraint = cmds.parentConstraint(layerJoint, joint, maintainOffset=transFilter, name=joint+'_all_FK_layer_parentConstraint')[0]
            scaleConstraint = cmds.scaleConstraint(layerJoint, joint, maintainOffset=transFilter, name=joint+'_all_FK_layer_scaleConstraint')[0]
...
def createModuleFromAttributes(moduleAttrsDict, createFromUI=False):
    ...
    if cmds.namespace(exists=moduleAttrsDict['module_Namespace']):
        ...
        currentCopyNamespacesForModule = filter(lambda namespace:re.match('^MRT_\D+__%s%s[0-9]*$'%(currentModuleUserSpecifiedNameBase, underscore_search), namespace), \
        ...
        moduleAttrsDict['module_Namespace'] = 'MRT_{0}__{1}{2}{3}'.format(moduleAttrsDict['node_type'], currentModuleUserSpecifiedNameBase, underscore_search, (suffix+1))
    ...
    if moduleAttrsDict['mirror_options'][0] == 'On':
        ...
        if cmds.namespace(exists=moduleAttrsDict['mirror_module_Namespace']):
            ...
            currentCopyNamespacesForModule = filter(lambda namespace:re.match('^MRT_\D+__%s%s[0-9]*$'%(currentModuleUserSpecifiedNameBase, underscore_search), namespace), updated_namespaces_for_mirror)
            ...
            moduleAttrsDict['mirror_module_Namespace'] = 'MRT_{0}__{1}{2}{3}'.format(moduleAttrsDict['node_type'], currentModuleUserSpecifiedNameBase, underscore_search, (suffix+1))
...
def returnValidSelectionFlagForModuleTransformObjects(selection):
    ...
    """
    matchObjects = [re.compile('^MRT_\D+__\w+:module_transform$'), re.compile('^MRT_\D+__\w+:root_node_transform$'),
                    re.compile('^MRT_\D+__\w+:end_node_transform$'), re.compile('^MRT_\D+__\w+:node_\d+_transform$'),
                    re.compile('^MRT_\D+__\w+:spline_\d+_adjustCurve_transform$'), re.compile('^MRT_\D+__\w+:splineStartHandleTransform$'),
                    re.compile('^MRT_\D+__\w+:splineEndHandleTransform$'), re.compile('^MRT_\D+__\w+:root_node_transform_control$'),
                    re.compile('^MRT_\D+__\w+:end_node_transform_control$'), re.compile('^MRT_\D+__\w+:node_1_transform_control$'),
                    re.compile('^MRT_\D+__\w+:single_orientation_representation_transform$'), re.compile('^MRT_\D+__\w+:[_0-9a-z]*transform_orientation_representation_transform$')]
